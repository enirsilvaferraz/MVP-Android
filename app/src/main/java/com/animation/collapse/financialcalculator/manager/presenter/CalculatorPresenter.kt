...
class CalculatorPresenter(view: CalculatorContract.View) : CalculatorContract.Presenter {
    ...
    val mBusiness: CalculatorContract.Business = CalculatorBusiness()
    ...
    override fun onCalculateClicked(value: String, interest: String, monthsCount: String) {
        ...
        if (value.toDoubleOrNull() == null) mView.showError("O campo valor é inválido")
        else if (interest.toDoubleOrNull() == null) mView.showError("O campo juros é inválido")
        else if (monthsCount.toIntOrNull() == null) mView.showError("O campo quantidade de meses é inválido")

        TaskGetResult(this).execute(CalculationVO(value, interest, monthsCount))
    }

    fun callService(calculationVO: CalculationVO) = mBusiness.calculate(calculationVO)

    fun showResult(calculationVO: CalculationVO?) {
        if (calculationVO != null) mView.setResult(calculationVO.getResultFormatted())
        else mView.showError("Problemas no cálculo")
    }
    ...
    private class TaskGetResult(val presenter: CalculatorPresenter) :
        ...
        override fun doInBackground(vararg params: CalculationVO?): CalculationVO =
             presenter.callService(params[0]!!)
